// <copyright file="NetFlowView.cs" company="Universität Stuttgart">
...
namespace DotNetFlow.Netflow9 {
    ...
    public sealed class NetflowView : NetflowViewBase<DataFlowSet,
            TemplateRecord, Field, FieldType> {
        ...
        public NetflowView(DataFlowSet data, TemplateRecord template)
            : base(data, template) { }
        ...
        public NetflowView(DataFlowSet data,
        ...
        /// <exception cref="ArgumentNullException">If
        /// <paramref name="template"/> is <c>null</c>.</exception>
        public NetflowView(DataFlowSet data, TemplateFlowSet template)
        ...
        public override dynamic this[int record] {
            get {
                ...
                for (var i = 0; i < this.Template.FieldCount; ++i) {
                    var name = this.Template.Fields[i].Type.ToString();
        ...
        protected override IList<Field> GetFields() {
            return this.Template.Fields;
        }

        /// <inheritdoc />
        protected override int GetIndex(FieldType field) {
            int retval = 0;
            while (this.Template.Fields[retval].Type != field) {
                ++retval;
            }
