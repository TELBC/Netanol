            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    foreach (var host in hosts)
                    {
                        ...
                        {
                            _logger.LogDebug("No DNS host found for IP address {IpAddress}. Defaulting to 'Unknown Hostname'", host.IpAddress);
                            resolvedDnsName = "Unknown Hostname";
                        }
                        catch (Exception)
                        {
                            _logger.LogDebug("Unexpected error resolving IP address {IpAddress}. Defaulting to 'Unknown Hostname'", host.IpAddress);
                            resolvedDnsName = "Unknown Hostname";
                        }
